...
function filterStagiaire() {
    ...
    promo = $("#Promotion").val();
    date_1 = $("#date_1").val() ? $("#date_1").val() : 'all';
    date_2 = $("#date_2").val() ? $("#date_2").val() : 'all';
    ...
    if (etat == 10)
        ...
        $("#promo").hide();
    // alert(niveau);
    var link_params = specialite + '/' + etablissement + '/' + genre + '/' + etat + '/' + niveau + '/' + date_1 + '/' + date_2 + '/' + promo + "/" + classe + "/" + annee_scolaire;
    var link_DT = racine + 'stagiaires/getStagiairesDT/' + link_params;
    var link_EXCEL = racine + 'stagiaires/exportExcel/' + link_params;
    $('#stagiairesExportExcel').attr("href", link_EXCEL);
    $('#datatableshow').attr("link", link_DT);
    $('#datatableshow').DataTable().ajax.url(link_DT).load();
}
//filter des sortants interface admin etablissement
function filterStagiaireByEtablissement() {
    var specialite = $("#specialite").val();
    var etablissement = $("#etablissement_id").val();
    var genre = $("#genre").val();
    var niveau = $("#niveau_form").val();
    var classe = $("#classes").val();
    var annee_scolaire = $("#annee_scolaire").val();
    var link_params = etablissement + '/' + specialite + '/' + genre + '/' + niveau + '/' + classe + '/' + annee_scolaire;
    var link_DT = racine + 'etablissements/get_etudiantDT/' + link_params;
    var link_EXCEL = racine + 'etablissements/stagiaires/exportExcel/' + link_params;
    $('#datatableshow').attr("link", link_DT);
    $('#datatableshow').DataTable().ajax.url(link_DT).load();
}
...
function addNewStagiare(etablissement = 'none') {
    $('#addNewModal #form-errors').hide();
    // var element = $(this);
    $("#addNewModal .form-loading").show();
    // var data = $('#addNewModal form').serialize();
    $.ajax({
        ...
        success: function (data) {
            ...
            if (etablissement == 'none') {
                ...
                // $('#datatableshow').DataTable().ajax.url(racine + "stagiaires/getStagiairesDT/all/all/all/all/all/all/all/all/all/" + data).load();
            } else {
                openStagiaireModal(data, 1);
                // $('#datatableshow').DataTable().ajax.url(racine + "etablissements/get_etudiantDT/" + etablissement + "/" + data).load();
            }
...
function openStagiaireModal(id, element = 0, showall = 0) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#de-modal .modal-dialog").addClass("modal-lg");
            $("#de-modal .modal-header-body").html(data);
            $("#de-modal").modal();
            // if (element == 0) {
                $('#datatableshow').DataTable().ajax.reload();
                // $('#datatableshow').DataTable().ajax.url(racine + "stagiaires/getStagiairesDT/" + specialite + '/' + etablissement + '/' + genre + '/' + etat + '/' + niveau + '/all/all/' + promotion + "/all/" + id).load();
...
function editStagiare() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // $('#datatableshow').DataTable().ajax.url(racine + "stagiaires/getStagiairesDT/all/all/all/all/all/all/all/all/all/all/" + data).load();
...
function openEvaluation(id_sortant, id_enquete) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#evaluation-modal .modal-dialog").addClass("modal-lg");
...
function stg_export() {
    var classe_val = $("#classes").val();
    window.location.href = racine + 'stg_export/' + classe_val;
}

...
function setDataTable(element) {
    ...
    if (!$.fn.dataTable.isDataTable(element) && $(element).length) {
        ...
        oTable = $(element).DataTable({
            ...
            "ajax": {"url":$(element).attr("link"),type},
            "columns": colonnes,
...
function saveform2(element, aftersave = null) {
    var container = $(element).attr('container');
    $('#' + container + ' #form-errors').hide();
    $(element).attr('disabled', 'disabled');
    $('#' + container + ' .main-icon').hide();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#' + container + ' .dcs-invalid-feedback').each(function (index, item) {
                $(item).remove();
            });
        },
        error: function (data) {
...
function getExport(form_id, link){
    alert(form_id,link);
    $('#'+form_id).attr('action', racine + link);

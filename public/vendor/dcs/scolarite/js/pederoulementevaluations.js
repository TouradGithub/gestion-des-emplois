$(document).ready(function () {
  ...
  setInterval( getEvaluationStat, 1000 );
});
...
function saveFormateurAnswer(reponse_id, groupe_id, valeur_id, values) {
    ...
    $.ajax({
        type: 'get',
        url: racine + 'evaluations/saveFormateurAnswer/' + reponse_id + '/' + groupe_id + '/' + valeur_id + '/' + values,
...
function getEvaluationStat() {
  if (($('#etatEvaluation').val() == 3 || $('#etatEvaluation').val() == 6) && $('#timeleft').val() > 0) {
      $.ajax({
          type: 'get',
          url: racine + 'evaluations/getEvaluationStat/' + $('#instance_id').val() + '/' + $('#timeleft').val(),
          success: function (data) {
              offline_tries = 0;
